"""
It's from the codebase of PatchGCN.
It converts the WSI graphs in a batch into a single graph, keeping the same with PatchGCN. 
"""
...
def collate_MIL_graph(batch):
    ...
    return [samples[0] if isinstance(samples[0], tuple) else default_collate(samples) for samples in transposed]


class GraphBatch(torch_geometric.data.Batch):
    def __init__(self):
